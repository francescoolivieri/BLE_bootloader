...
# TODO: verify checksum and counter if correct
async def read_response(sender, data):
    ...
    if not security:
        ...
        if (data[0] >> 4) == 0x0 and dec_payload != False:

            rand_IV = dec_payload[2:]
            #rand_IV = [rand_IV[0].to_bytes(1, byteorder='big'), rand_IV[1].to_bytes(1, byteorder='big'), rand_IV[2].to_bytes(1, byteorder='big')]
            #print(f"{rand_IV[0]},{rand_IV[1]},{rand_IV[2]}")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowIcon(QIcon("icona_app.png"))
        ...
        self.log_textbox.setFixedSize(390, 80)
    ...
    async def send_new_fw(self):
        ...
        async with BleakClient(self.device_address_textbox.text()) as client:
            ...
            self.update_log("Settings completed\n")
            ...
            while connection_status != ConnectionStatus.CLOSING_PHASE:
                if connection_status == ConnectionStatus.INIT_PHASE:  # START FLASH MODE PACKET
                    ...
                    cont_chunck = 0
                    while cont_chunck < num_chunks:
                        chunk = chunks[cont_chunck].ljust(14, b"0x00"[0:1])
                        cont_chunck_bytes = cont_chunck.to_bytes(2, byteorder='big')

                        msg = bytearray()
                        msg.append((0xc << 4) | (cont_chunck_bytes[0] & 0x0f))
                        msg.append(cont_chunck_bytes[1])

                        # TODO calc new IV with sha3: hashlib.sha3_224(IV || input_data).hexdigest() IF FRESH
